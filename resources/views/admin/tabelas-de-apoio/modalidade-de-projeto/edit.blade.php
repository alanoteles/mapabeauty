...
@section('content')
    ...
    {{ Form::open(
       array(
           'url'   => App::getLocale() . '/admin/tabelas-de-apoio/modalidade-de-projeto/' .  (isset($resultado->id) ? $resultado->id : '' ),
           'name'  => 'frm',
           'id'    => 'frm',
    ...
    {{ Form::hidden('model', (isset($model) ? $model : ''), array('id' => 'model')) }}
        ...
        <div class="col-xs-12 botoes-pj-pf">
            @if(isset($resultado->id)) <button type="button" class="btn remover-item margin-right-20 margin-left-2 excluir" data-id="{{$resultado->id}}">Excluir modalidade</button> @endif
            <a href="" data-route="/admin/tabelas-de-apoio/modalidade-de-projeto"  class="btn cancelar btn-sm font-size-15 margin-right-2" style="padding: 3px 10px 4px 10px;">Cancelar</a>
